...
def add_frame_axes(frame_bgr, tick_interval=250, tick_length=6, margin=60,
                   color=(0, 0, 0), bg_color=(255, 255, 255), font_scale=0.5, thickness=1):
    ...
    for x in range(0, w+1, tick_interval):
        ...
        cv2.putText(canvas, label, (cx - lw//2, img_top - tick_length - 4),
    ...
    for y in range(0, h+1, tick_interval):
        ...
        label = str(y)
        (lw, lh), _ = cv2.getTextSize(label, font, font_scale, thickness)
        cv2.putText(canvas, label, (img_left - tick_length - lw - 4, cy + lh//2),
                    font, font_scale, color, thickness, cv2.LINE_AA)
    return canvas
...
def save_screenshot(frame_bgr, frame_count, output_dir, box_w, box_h,
                    center_x, center_y, axes_margin=60):
    os.makedirs(output_dir, exist_ok=True)
    ...
    path = os.path.join(output_dir, f"frame_{frame_count:04d}_{to_polarspec_timestamp()}.png")
...
def capture_frames(camera_index=0, test_length=10.0, show_preview=False,
    ...
    print(f"Camera opened: {w}x{h} | capturing {int(test_length)} frame(s) over {test_length:.0f}s")
    ...
    try:
        while True:
            ...
            if now >= next_capture:
                ...
                append_avg_rgb(txt_file, frame_rgb, frame_count, ts, box_w, box_h, cx, cy)

                if screenshot_interval and frame_count % screenshot_interval == 0:
                    save_screenshot(frame_bgr, frame_count, screenshot_dir,
                                    box_w, box_h, cx, cy)

                print(f"  Frame {frame_count:4d} | t={elapsed:5.1f}s | {test_length - elapsed:.1f}s remaining")
